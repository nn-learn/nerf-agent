...
## V4.4：多轮评测与 fail-closed 发布门

V4.4 使用 30 条、共 78 个 turn 的确定性多轮工程轨迹，不使用 LLM 充当裁判。轨迹覆盖目标所有权、Memory 目标注入、阶段迁移、危机覆盖、两轮同意、同意限域、拒绝/停止/过期、冷却、用户自报反馈、漂移标志和遥测内容泄漏。

所有工程契约当前均通过，发布门返回 `ENGINEERING_DEMO_COMPLETE`、`demo_ready=true`、`production_ready=false`。生产门保持关闭，直到至少 500 条独立标注留出轨迹、临床与隐私签字、独立红队、数字人可访问性、危机演练、部署隐私审计、代表设备延迟、纵向监控 runbook、同意用户研究、干预证据复核和事件响应演练全部完成。指标定义和复现见 [AGENT_V4_EVALUATION.md](./AGENT_V4_EVALUATION.md)。
